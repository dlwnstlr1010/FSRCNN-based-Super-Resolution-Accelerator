...
//    output wire             ila_pause,
//    output wire             ila_fifo_empty,
//    output wire             ila_started,
//    output wire     [10:0]  ila_fifo_x_cnt,
//    output wire             ila_not_ready,
//    output wire             ila_padding
    );
    ...
    if(!axi_reset_n) begin
        ...
        if(bram_x_cnt <= 319) begin
            ...
            if(ready_delay == 3) begin
                if((s_axis_tvalid || pause || padding) && !not_ready) begin
...
always @(posedge axi_clk or negedge axi_reset_n) begin //ADDER TREE CONTROL
    if(!axi_reset_n) begin
        ...
        if(requant_valid) begin
            output_o <= (requant[24]) ? 8'd255 : requant[23:16]; //prevent overflow
            output_valid_o <= 1;
        end
        else begin
             output_o <= 0;
...
//        last_addr              <= 0;
        ...
        if (EOF) begin
            started <= 0;                  // EOF 들어오면 무조건 0
        end
        else if (s_axis_tvalid) begin
            started <= 1;                  // valid 들어오면 다시 1
        end
        else begin
            started <= started;            // 나머지는 현상 유지
        end
    end
end
...
//	.probe2(s_axis_tdata), // input wire [127:0]  probe2 
//	.probe3(m_axis_tready), // input wire [0:0]  probe3 
//	.probe4(m_axis_tvalid), // input wire [0:0]  probe4 
//	.probe5(m_axis_tdata), // input wire [7:0]  probe5 
//	.probe6(fifo_x_cnt), // input wire [10:0]  probe6 
//	.probe7(fifo_y_cnt), // input wire [9:0]  probe7 
//	.probe8(ready_delay), // input wire [4:0]  probe8 
//	.probe9(row_cnt), // input wire [1:0]  probe9 
//	.probe10(pause), // input wire [0:0]  probe10 
//	.probe11(not_ready), // input wire [0:0]  probe11 
//	.probe12(padding), // input wire [0:0]  probe12 
//	.probe13(fifo_empty), // input wire [0:0]  probe13 
//	.probe14(started) // input wire [0:0]  probe14
//);
